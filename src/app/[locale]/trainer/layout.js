...
export default function TrainerLayout({ children }) {
  const { user, loading, isTrainer, isAdmin, isSuperAdmin } = useAuth();
  ...
  useEffect(() => {
    if (!loading && (!user || !(isTrainer || isAdmin || isSuperAdmin))) {
      router.push(`/${locale}/login`);
    }
  }, [user, loading, isTrainer, isAdmin, isSuperAdmin, router, locale]);
  ...
  if (!user || !(isTrainer || isAdmin || isSuperAdmin)) return null;
  ...
  return (
    <TenantProvider>
      <div className="app-layout">
        ...
        <AIChatWidgetSafe locale={locale} role="trainer" />
      </div>
    </TenantProvider>
  );
}
