...
export default function ClientbookingsclassesPage() {
  ...
  return (
    <div className="animate-fadeIn">
      <div className="page-header"><h1><span>📅</span> {isAr ? 'حجز حصة' : 'Book Class'}</h1></div>
      <div className="card" style={{ textAlign: 'center', padding: 'var(--space-8)' }}>
        <div style={{ fontSize: '4rem', marginBottom: 'var(--space-4)' }}>📅</div>
        <h3 style={{ marginBottom: 'var(--space-2)', color: 'var(--pt-gray-300)' }}>{isAr ? 'لا توجد حجوزات' : 'No bookings yet'}</h3>
        <p style={{ color: 'var(--pt-gray-500)', fontSize: 'var(--font-size-sm)' }}>{isAr ? 'احجز حصة تدريبية' : 'Book a training class'}</p>
      </div>
    </div>
  );
}
