...
export default function ClientgoalsPage() {
  ...
  return (
    <div className="animate-fadeIn">
      <div className="page-header"><h1><span>🎯</span> {isAr ? 'الأهداف' : 'Goals'}</h1></div>
      <div className="card" style={{ textAlign: 'center', padding: 'var(--space-8)' }}>
        <div style={{ fontSize: '4rem', marginBottom: 'var(--space-4)' }}>🎯</div>
        <h3 style={{ marginBottom: 'var(--space-2)', color: 'var(--pt-gray-300)' }}>{isAr ? 'لم يتم تحديد أهداف بعد' : 'No goals set yet'}</h3>
        <p style={{ color: 'var(--pt-gray-500)', fontSize: 'var(--font-size-sm)' }}>{isAr ? 'حدد أهدافك لتتبع تقدمك' : 'Set your goals to track your progress'}</p>
      </div>
    </div>
  );
}
