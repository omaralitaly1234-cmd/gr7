...
export default function ClientDietPlanPage() {
  ...
  if (!dietPlan) return (
    <div className="animate-fadeIn">
      <div className="page-header"><h1><span>🥗</span> {isAr ? 'خطة التغذية' : 'Diet Plan'}</h1></div>
  ...
  return (
    <div className="animate-fadeIn">
      ...
      <div className="card" style={{ marginBottom: 'var(--space-6)', borderTop: '3px solid var(--pt-gold)' }}>
        <h2 style={{ fontSize: 'var(--font-size-xl)', marginBottom: 'var(--space-2)' }}>{dietPlan.name || (isAr ? 'خطة التغذية' : 'Diet Plan')}</h2>
        {dietPlan.calories && <div style={{ marginTop: 'var(--space-3)', display: 'flex', gap: 'var(--space-4)', fontSize: 'var(--font-size-sm)' }}><span style={{ color: 'var(--pt-gold)', fontWeight: 700 }}>🔥 {dietPlan.calories} {isAr ? 'سعرة' : 'cal'}</span>{dietPlan.protein && <span>💪 {dietPlan.protein}g</span>}{dietPlan.carbs && <span>🍞 {dietPlan.carbs}g</span>}{dietPlan.fat && <span>🥑 {dietPlan.fat}g</span>}</div>}
      </div>
      {meals.length > 0 ? (
