...
export default function ClientspabookingPage() {
  ...
  return (
    <div className="animate-fadeIn">
      <div className="page-header"><h1><span>🧖</span> {isAr ? 'حجز السبا' : 'Spa Booking'}</h1></div>
      <div className="card" style={{ textAlign: 'center', padding: 'var(--space-8)' }}>
        <div style={{ fontSize: '4rem', marginBottom: 'var(--space-4)' }}>🧖</div>
        <h3 style={{ marginBottom: 'var(--space-2)', color: 'var(--pt-gray-300)' }}>{isAr ? 'لا توجد حجوزات سبا' : 'No spa bookings'}</h3>
        <p style={{ color: 'var(--pt-gray-500)', fontSize: 'var(--font-size-sm)' }}>{isAr ? 'احجز جلسة سبا من هنا' : 'Book a spa session here'}</p>
      </div>
    </div>
  );
}
