...
export default function ClienttransformationPage() {
  ...
  return (
    <div className="animate-fadeIn">
      <div className="page-header"><h1><span>🔄</span> {isAr ? 'التحول' : 'Transformation'}</h1></div>
      <div className="card" style={{ textAlign: 'center', padding: 'var(--space-8)' }}>
        <div style={{ fontSize: '4rem', marginBottom: 'var(--space-4)' }}>🔄</div>
        <h3 style={{ marginBottom: 'var(--space-2)', color: 'var(--pt-gray-300)' }}>{isAr ? 'لا توجد صور تحول' : 'No transformation photos'}</h3>
        <p style={{ color: 'var(--pt-gray-500)', fontSize: 'var(--font-size-sm)' }}>{isAr ? 'التقط صور قبل وبعد لتتبع تحولك' : 'Take before/after photos to track progress'}</p>
      </div>
    </div>
  );
}
